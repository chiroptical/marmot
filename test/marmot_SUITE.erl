...
simple_types(_Config) ->
    {ok, [int, bit_array, uuid, bool]} =
        resolve(~"select $1::integer, $2::text, $3::uuid, $4::bool").

array_of_int(_Config) ->
    {ok, [{list, int}]} = resolve(~"select $1::integer[]").

array_of_text(_Config) ->
    {ok, [{list, bit_array}]} = resolve(~"select $1::text[]").

enum_param(_Config) ->
    {ok, [{enum, ~"mood", [~"happy", ~"sad", ~"meh"]}]} = resolve(~"select $1::mood").

enum_array_param(_Config) ->
    {ok, [{list, {enum, ~"mood", [~"happy", ~"sad", ~"meh"]}}]} = resolve(~"select $1::mood[]").

enum_mixed(_Config) ->
    {ok, [{enum, ~"mood", [~"happy", ~"sad", ~"meh"]}, int, bit_array]} =
        resolve(~"select $1::mood, $2::integer, $3::text").

two_distinct_enums(_Config) ->
    {ok, [
        {enum, ~"mood", [~"happy", ~"sad", ~"meh"]}, {enum, ~"color", [~"red", ~"green", ~"blue"]}
    ]} =
        resolve(~"select $1::mood, $2::color").

enum_tricky_labels(_Config) ->
    {ok, [{enum, ~"weird", [~"a-b", ~"not allowed", ~"UPPER"]}]} =
        resolve(~"select $1::weird").
...
resolve(SQL) ->
    {ok, ParamOids, _Fields} = protocol:prepare_statement(SQL),
