...
-export([module/2, forms/2, render/1, banner/0, format_error/1]).

-export_type([reason/0]).

-type reason() ::
    {duplicate_output_columns, file:filename_all(), [atom()]}
    | {enum_conflict, binary(), [pos_integer()]}
    | {name_collision, atom(), arity()}.

-define(BANNER, ~"%% Code generated by marmot. DO NOT EDIT.\n\n").
-define(PRETTYPR_OPTS, [{paper, 96}, {ribbon, 90}]).
-define(MERL_POS, 1).

-spec module(module(), [#typed_query{}]) -> {ok, binary()} | {error, reason()}.
...
-spec forms(module(), [#typed_query{}]) ->
    {ok, [erl_parse:abstract_form()]} | {error, reason()}.
    ...
    maybe
        ...
        Header =
            [
                ...
                lists:flatmap(fun record_forms/1, Queries),
...
render(Forms) ->
    ...
    marmot_error:message(Formatted).
...
-spec validate([#typed_query{}]) -> ok | {error, reason()}.
validate(Queries) ->
    maybe
...
-spec check_duplicate_output_columns([#typed_query{}]) -> ok | {error, reason()}.
...
-spec check_enum_conflicts([#typed_query{}]) -> ok | {error, reason()}.
...
-spec check_name_collisions([#typed_query{}]) -> ok | {error, reason()}.
